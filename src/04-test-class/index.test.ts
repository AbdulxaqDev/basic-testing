...
describe('BankAccount', () => {
  ...
  test('should throw InsufficientFundsError error when withdrawing more than balance', () => {
    expect(() => firstAccount.withdraw(firstBalance + 5)).toThrow(
      `Insufficient funds: cannot withdraw more than ${firstBalance}`,
    );
  });
  ...
  test('should throw error when transferring more than balance', () => {
    ...
    ).toThrow(`Insufficient funds: cannot withdraw more than ${firstBalance}`);
  });

  test('should throw error when transferring to the same account', () => {
    expect(() => firstAccount.transfer(firstBalance, firstAccount)).toThrow(
      `Transfer failed`,
    );
  });
